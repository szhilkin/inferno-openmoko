...
//        fprint(fd,"<%s 0x%X %02X:", t->comment, t->size, t->np); \
//        for(i=0; i<t->np; i++) fprint(fd,"%02X", (uchar)t->map[i]); \
//        fprint(fd,">"); \
//        }
#define PRINT_TYPE(fd,t) {int i; \
        fprint(fd,"<%s 0x%X %02X:", "", t->size, t->np); \
        for(i=0; i<t->np; i++) fprint(fd,"%02X", (uchar)t->map[i]); \
        fprint(fd,">"); \
        }
...
#define H               ((void*)(-1))
//#define H2D(t, x)       ( (t) (assert((x)!=nil), assert((x)!=H), ((Heap*)(x)+1)) )
//#define D2H(x)          ( assert((x)!=nil), assert((x)!=H), ((Heap*)(x)-1) )
